...
`layout: 'elk'` requires the ELK import and registration. If ELK is required and unavailable, block generation. Use dagre only when it was selected from the start.
...
| -------------- | ------- | ------------------------------------ |
| `-->`          | Solid   | Primary flow                         |
| `-.->`         | Dotted  | Optional or async paths              |
