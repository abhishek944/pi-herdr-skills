...
3. Parse every returned tab and pane ID from Herdr's JSON responses. Never predict an ID or derive it from layout order. Before starting agents, use `herdr pane rename` to give the root pane and every split pane a distinct name describing its assigned perspective; pane names are mandatory.
...
5. Preserve the caller's focus throughout. On success or failure, close only the task tab created by this run; never close the caller's tab, pane, workspace, or session. If Herdr setup fails after creating the tab, clean up that tab and block the council.
6. If Herdr cannot be used or any required agent fails, clean up owned resources and block the council.
