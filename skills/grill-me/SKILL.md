...
Herdr-hosted Pi agents are the only runtime. Require `HERDR_ENV=1` and the `herdr` command, then read and follow the globally installed `herdr` skill before issuing control commands. Record the caller workspace, tab, pane, working directory, and focus state. Partition concurrent agents into ordered groups of at most four and create one dedicated task tab per group in the caller's workspace with the caller's working directory and `--no-focus`. Parse every created tab and root-pane ID from Herdr's JSON; never guess IDs. Keep each root pane for the group's first agent and follow the Herdr skill's required split order, producing a 2x2 grid for four-agent groups. Never create a fifth pane in a tab; additional groups always use new tabs. Before starting agents, give every pane a distinct responsibility name, apply the Herdr skill's bounded readiness gate to every newly created pane across all groups, and make the complete batch ready. Start only Pi agents with unique names and explicit resolved provider/model/thinking settings. Save and verify every start response before prompting. An `agent_pane_busy` response after readiness follows the Herdr skill's fail-closed cleanup path and is not model fallback. Run prompts concurrently, give every productive prompt-and-wait deadline at least `900000` milliseconds (15 minutes), and collect every complete result. Herdr setup that remains failed after bounded readiness recovery, ambiguous prompt/wait failure, output loss after work, or failed provenance blocks after owned-resource cleanup. A confirmed no-contribution automatic-model failure follows the shared fresh-pane fallback flow instead of blocking immediately.
