...
3. After the contract preflight passes, start one uniquely named Pi agent in each named pane with the Herdr skill's mandatory `--kind pi`, passing that reviewer's resolved `--provider`, `--model`, and `--thinking` Pi arguments after `--`. Save each complete start response and verify it through the shared model-routing policy before prompting. A startup or launch-verification failure blocks after exact cleanup because it cannot prove the immutable session needed for fallback. Start and verify **all** required reviewer agents before any review prompt or wait begins. Do not focus the review tab.
