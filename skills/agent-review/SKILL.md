...
4. After every agent has started, launch all three isolated `herdr agent prompt <name> <prompt> --wait --timeout <same-deadline>` commands concurrently, with separate output/error capture. Each command submits its prompt and observes the required lifecycle change before waiting for a settled state, avoiding an idle-before-work race. Join the prompt-and-wait processes as a group rather than serially. A blocked, timed-out, unknown, stalled, or failed reviewer is not a clean result.
