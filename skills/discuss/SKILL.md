...
2. Capture the caller's working directory and Herdr context. Partition concurrent subagents into ordered groups of at most four. In the current workspace, create one dedicated unfocused task tab per group, preserving the working directory. Use each tab's root pane for the group's first subagent and follow the Herdr skill's required split order to produce a 2x2 grid for every four-agent group. Never create a fifth pane in a tab; additional groups always use additional tabs.
3. Parse every returned tab and pane ID from Herdr's JSON responses. Never predict an ID or derive it from layout order. Before starting agents, use `herdr pane rename` to give every root and split pane a distinct name describing its evidence track; pane names are mandatory. Apply the Herdr skill's bounded readiness gate to every newly created pane across every group and make the full batch ready before starting any agent.
4. Start one Pi subagent per named pane using the Herdr skill's mandatory `--kind pi`, passing its resolved `--provider`, `--model`, and `--thinking` Pi arguments after `--`; never launch or substitute another agent kind. Save and verify every start response through the shared model-routing policy. An `agent_pane_busy` response after readiness follows the Herdr skill's fail-closed cleanup path and is not model fallback. After all agents start, run one `herdr agent prompt ... --wait --timeout <milliseconds>` command per agent concurrently, using at least `900000` milliseconds (15 minutes), then collect every output before synthesis.
5. Preserve the caller's focus throughout. On success or failure, close only the exact task tabs created by this run; never close the caller's tab, pane, workspace, or session. If Herdr setup still fails after bounded readiness recovery, clean up every owned task tab and block the investigation.
